...
const MODE_CAPABLE={pullup:['force','endurance','power'],pushup:['force','endurance','power'],row:['force','endurance','power'],split:['force','endurance','power'],pike:['force','endurance','power'],dip:['force'],leg:['force'],bridge:['force','endurance']};
...
export function buildTechnique({exerciseId,lineId,modeId,variant,unit}){
  ...
  const type=typeFor(exerciseId,variant.id,modeId,unit),allowed=MODE_CAPABLE[exerciseId]||[];
  const compatibleModes=type==='dynamic_controlled'||type==='explosive'?allowed.filter(m=>m===modeId||((exerciseId==='pushup'||exerciseId==='pullup')&&['classique','stricte'].includes(variant.id))):[];
  const addition=ADDITIONS[`${exerciseId}:${variant.id}`]||[];
  const modeInstructions=compatibleModes.includes(modeId)?[...MODE_INSTRUCTIONS[modeId]]:[];
  const position=[...base.position],execution=[...base.execution,...addition],mistakes=[...base.mistakes],stop=variant.stop||base.stop;
  const sessionCues=[position[0],execution[0],modeInstructions[0],stop].filter(Boolean).slice(0,4);
  return{id:variant.id,exerciseId,lineId,name:variant.name,type,position,execution,mistakes,stop,compatibleModes,modeInstructions,unilateral:unit==='side'||unit==='side-seconds',timed:unit==='seconds'||unit==='side-seconds',sessionCues};
}
